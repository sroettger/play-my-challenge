...
with open('socket.io.min.js') as f:
  socketio_js = f.read()

async def index(request):
  return web.Response(text=index_html, content_type='text/html')

async def socketio(request):
  return web.Response(text=socketio_js, content_type='text/javascript')

app.add_routes([web.get('/', index),
                web.get('/socket.io.min.js', socketio),
                web.get('/{name}', index)])
...
async def create_gdb_process(sid):
  ...
  gdb_sessions[sid] = await GDBProcess.create(runnable, stop_queue, env={}, log_fn=log)
